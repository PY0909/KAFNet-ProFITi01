...
for path in (CODE_ROOT, BASELINE_ROOT):
...
def _run_dir(config: BaselineConfig) -> Path:
    return Path(config.output_dir) / config.run_id


def _artifact_dir(config: BaselineConfig, artifact: str) -> Path:
    return _run_dir(config) / artifact / config.dataset / config.model
...
def _write_config_yaml(path: Path, config: BaselineConfig) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _checkpoint_payload(model, config: BaselineConfig, history, best_record=None):
    return {
...
def _build_model(config: BaselineConfig, num_sensors: int, context_dim: int):
...
def _finite_labels_scores(labels, scores):
...
def _fit_platt(labels, scores):
    ...
    if labels.size == 0 or np.unique(labels).size < 2 or LogisticRegression is None:
        return {"coef": 1.0, "intercept": 0.0, "status": "identity_fallback"}
    try:
...
def _apply_compare_risk_calibration(scores, calibration: Optional[Dict[str, object]]):
    if calibration is None:
        return scores
    orientation = calibration.get("risk_orientation", "direct")
    oriented = 1.0 - scores if orientation == "inverted" else scores
    platt = calibration.get("platt", {"coef": 1.0, "intercept": 0.0})
    coef = float(platt.get("coef", 1.0))
    intercept = float(platt.get("intercept", 0.0))
    if torch.is_tensor(oriented):
        calibrated = torch.sigmoid(oriented * coef + intercept)
        return calibrated.clamp(0.0, 1.0)
    return np.clip(_sigmoid(oriented * coef + intercept), 0.0, 1.0)


def _fit_compare_risk_calibration(labels, scores, quantile: float = 0.95):
    labels, scores = _finite_labels_scores(labels, scores)
    if labels.size == 0:
        return {
            ...
            "platt": {"coef": 1.0, "intercept": 0.0, "status": "identity_fallback"},
        }
    metrics = safe_binary_metrics(labels, scores, allow_orientation_selection=True) if np.unique(labels).size >= 2 else None
    orientation = metrics.get("risk_orientation") if metrics else "direct"
    if orientation is None:
        orientation = "direct"
    oriented_scores = 1.0 - scores if orientation == "inverted" else scores
    platt = _fit_platt(labels, oriented_scores)
    calibrated = _apply_compare_risk_calibration(scores, {"risk_orientation": orientation, "platt": platt})
    normal_mask = labels < 1
    normal_scores = calibrated[normal_mask] if normal_mask.any() else calibrated
    threshold = float(np.quantile(normal_scores, float(quantile))) if normal_scores.size else 0.5
    return {
        ...
        "validation_risk_score_mean": float(np.mean(calibrated)),
        "validation_normal_score_quantile": threshold,
        "validation_normal_count": int(normal_mask.sum()),
    }


def _train_epoch(model, loader, optimizer, device: torch.device, config: BaselineConfig):
    model.train()
    total = 0.0
    count = 0
...
def _evaluate(
        ...
        for idx, batch in enumerate(loader):
            ...
            if prediction_writer is not None:
                samples = model.sample(
                    ...
                    t_obs=batch.T_obs,
                    t_q=batch.T_q,
                ).clamp(-config.sample_clip, config.sample_clip)
    ...
    averaged["risk_source"] = f"{config.model}_mean_scale"
    return averaged


def _collect_validation_calibration(model, loader, device: torch.device, config: BaselineConfig):
    risks = []
    labels = []
    model.eval()
    with torch.no_grad():
        for idx, batch in enumerate(loader):
            ...
            risks.append(model.risk_score(mean, scale).detach().cpu().numpy())
            labels.append(_risk_labels(config.dataset, batch, config.risk_threshold).detach().cpu().numpy())
    risk_scores = np.concatenate(risks) if risks else np.array([])
    risk_labels = np.concatenate(labels) if labels else np.array([])
    calibration = _fit_compare_risk_calibration(risk_labels, risk_scores)
    calibration.update(
        {
            ...
            "risk_calibration_note": "Platt is fitted on validation scores; threshold uses validation normal q=0.95; no test labels are used.",
            "conformal_qhat": 0.0,
            "conformal_qhat_source": "not_used_by_compare_baseline",
        }
    )
    return calibration


def run_experiment(config: BaselineConfig):
    if config.model not in list_baseline_models():
        raise ValueError(f"Unknown model {config.model}; choose from {list_baseline_models()}")
    torch.manual_seed(config.seed)
    np.random.seed(config.seed)
    if not config.run_id:
        config.run_id = f"{config.dataset}_{config.model}_{_timestamp_run_id()}"
    ...
    split_info["compare_protocol"] = {
        "dataset_split": "shared",
        "missing_mask": "shared_npz",
        "risk_label_rule": "shared_project_rule",
        "risk_threshold_rule": "validation_normal_q95",
        "platt_calibration": "validation_only",
        "checkpoint_selection": "best_valid_crps",
    }
    ...
    model = _build_model(config, bundle.num_sensors, bundle.context_dim).to(device)
...
def parse_args() -> BaselineConfig:
    parser = argparse.ArgumentParser(description="Train/evaluate unified probabilistic baselines")
    parser.add_argument("--dataset", required=True)
    parser.add_argument("--model", required=True, choices=list_baseline_models())
    ...
    parser.add_argument("--te-dim", type=int, default=10)
    parser.add_argument("--kernel-count", type=int, default=4)
    parser.add_argument("--n-layers", type=int, default=2)
    parser.add_argument("--preconv-dim", type=int, default=16)
    ...
    return BaselineConfig(**vars(parser.parse_args()))
